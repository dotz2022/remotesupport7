            ...
            string onCallSMEUsers = "")
        {
            ...
            if (searchServiceResults != null)
            {
                foreach (var ticket in searchServiceResults)
                {
                    ...
                    {
                        Title = $"<b>{HttpUtility.HtmlEncode(ticket.Title)} | {HttpUtility.HtmlEncode(ticket.Severity == (int)TicketSeverity.Callback ? localizer.GetString("UrgentText") : localizer.GetString("NormalText"))}</b>",
